...
The servers then boot with PXE on this image (see below for more explanations),
...
However, the real problem was the management with IaC.
...
Ansible is stateless, unlike terraform for example. The idea is to
...
To sum steps constructed with `overlayfs`, what better way than to use
...
function, there is however a mechanism to manage the matching between versions of the
kernel (and associated initramfs) and the version of the kernel modules + sources in
the rootfs.

Indeed, if we build the kernel+initramfs to have the latest version,
but the rootfs has not been rebuilt for a while and uses an old
...
It is relevant to pool them to avoid having too much duplication.
...
The conffs should not contain kernel modules or
modules of the rootfs to avoid breaking this mechanism which does not allow to have
granularity of the order of the host.
{: .notice--info }

## Integration with the docker registry

Because all our rootfs and conffs are hierarchically constructible and
via Docker, it is also interesting to use a docker registry to
store the final images ready for export, and the images
intermediaries.

To do this, all `docker' commands are actually wrapped in a
function of `utils.sh`, which takes care of :
- Automatically log in to the registry with the values defined in the
  ...
  much more in the IC for example
- Retag the image during a docker build to have in addition
...
This allows us to have a tool that works between several
hosts and CI runner, and not to fear local deletion of a
docker image.

# How do you put it all together?

The question now is how to boost these resources,
and how they fit together.

This article is not intended to present in detail how to build a stack
PXE and dhcp, so we will assume that this aspect is already covered.

## PXE boot options

Let's look at an example of how to configure grubnet to boot into PXE on a master
with his conffs :
...
Well, the configuration seems a bit hairy on first approach, of course.

The basic idea is to indicate via the `linux` and `initrd` directives where to find
kernel and initramfs, but also to configure the kernel via the command line.

The first variables are normally relatively simple to understand,
on the other hand `boot_opts` deserves some further explanation.

Before we get into that, let's look at the other directives given to `linux`.

The `root` option will be used by initramfs to find out where to find rootfs.
...
we prefix with `live:` to indicate that we are going to boot on a live image,
found on the network and placed in RAM. The address is that of our rootfs, via
...
The `pxemac` and `ip` options allow us to force a network configuration
initramfs, here using the eth0 interface to find an IP
with the specified MAC. This setting may need to be adjusted depending on
the infra. The `net_default_mac` variable, on the other hand, must be
changed from one machine to another, quite logically.
...
but especially dracut. The options in `rd...` are dracut options (see
See the `dracut.cmdline(7)` documentation page for details).

Here we explicitly disable options that we do not need,
to save time at boot by not checking things that don't exist
in our setup, like `luks`, `md` or `lvm`.

On the other hand, the need for a network (`rd.neednet=1`) is indicated, and the
the fact that we want to have a rootfs that is writable as well.

### Have a read-write system

One of the limitations of booting live on a squashfs rootfs is the inability to
writing. A system with a read-only `/`partition is much more expensive than a system with a read-only partition.
too restrictive, it will be necessary to be cunning.

To be able to write to our score in read-only mode, it would be interesting to
to have a system where the squashfs can be read, but the writing is done on the
would be done elsewhere, in a place where one can write, but that the representation
The final `/` is the sum of the read-only + the changes made.

If this sounds familiar, it's normal, I'm describing overlayfs once again.

We just need to make an overlayfs with our rootfs as the bottom layer, in
layer a `tmpfs` and mount it merged on `/` to have a system
in reading/writing.

While we're at it, we might as well add on our conffs, since it's also
a simple method of incorporating its changes into the stack.
...
a boot, thanks to overlayfs. Just look at the contents of the `upperdir`.
{: .notice--info }
...
that already exist and can be found on their repo git.
...
not able to read this file, this is the role of `ci.py`, the script executed
in the IC -- which can also be run manually -- which takes care of reading
the configuration and invoke vauban with the correct options.
...
possibility to specify the branch to be used for the Master in general by
plus specify it for a `stage'. Consider the following example:
...
To overcome this, it is best to use the `ci.py` script, so as not to
not have to set the vauban invocation options on the CLI, but use
those in `config.yml`.

The script has been designed to work via Gitlab-CI, but is also
usable locally. The script takes its arguments by environment variable
(as it is designed to work in CI), and automatically reads `config.yml` for
deduct the rest.
...
- `debug`: If set to `true` or `yes`, does not run vauban.sh but displays the
   orders that will be executed

For `build_parent`, the parent of a master (rootfs) is the master (rootfs) it inherits from.
{: .notice--info }

For `build_parent`, the parent of a conffs and an initramfs is the associated rootfs.
{: .notice--info }

For `build_parent`, if `stage` is set to `all`, then `stage=all` will also be applied to the master(s) it inherits from
{: .notice--info }
...
options in `config.yml`, but are generic.
...
It is not possible to use an environment variable in
of hook pre/post ansible
{: .notice--warning }

A variable defined in `.secrets.env` cannot be overwritten with a
...
One of the advantages of Vauban, as mentioned above, is its approach to the
stateless and IaC. Thanks to the way we build our image, it is
also possible to add traceability to operations that have been
...
- `/imginfo`, which contains, in YAML format, the playbook application list
  ansible, with the date, the git branch, the commit hash and the hostname of the
  machine that was used.
- `/packages`, also in YAML format, which contains the differences on
  the list of installed packages, step by step.

It is therefore possible from any docker image, or directly
in the booted VM to consult these files for information on the
...
If the interest of the project is to be able to build "off-line" images,
and hierarchical, one of the major drawbacks is the application
of Ansible in a Docker container.
...
need to provide a condition like: `when: ansible_connection|default('dummy') != 'local'`.
Another possibility would be to define in `group_vars/all.yml` a variable
vauban: true` in the pre-answerable vauban hooks, and filter with
when: not vauban|default('false')`.

## Persistent volumes

One of the weaknesses of vauban at present is the management of persistent volumes.

In the same way as containers, which can be assimilated to our images
bootable in PXE, it is possible to have persistent storage by mounting
...
This operation cannot be done directly from vauban, as it is
specific to the VM.

If now the disk is ready, and mounted in `/srv/my-custom-app`. Default,
the configuration files that were put in this folder will not exist,
since it is located "below" the edit point, in the image and not on the disc.

The simplest solution is still to apply our Ansible for a second time
once, on the newly created VM to bootstrap the VM.

Once this is done, the VM behaves as expected, and perhaps
restarted without any problems.

For this reason, it is preferable to mount only persistent data,
and not the configuration, on the volumes. The configuration can be put on the
...
I was also able to give a [lecture available on youtube](https://www.youtube.com/watch?v=Mmzsg66k1KE)
presenting the project at the Info Night 2021.

If you have any other questions or comments, I will be happy to send them to
github, or by [mail](/about/#contact).
