...
Le projet vauban a été conçu dans le cadre de mon travail à [Diabolocom](https://www.diabolocom.com/fr/).
...
virtuelles boot en PXE sur une image. Cette image, générée à partir d'une machine
...
master qu'il y a de "type" de serveurs. Par exemple, nous aurons un master
...
Le serveurs bootent ensuite en PXE sur cette image (voir plus bas pour plus
d'explications), l'administration est en réalité assez analogue à la gestion
de conteneurs.

La manière d'administrer ces masters, et la génération des images à partir du
master présentaient plusieurs failles, que le projet vauban résoud.

## Problématiques et résolution
### Le problème du master stateful et IaC

Un des principaux problème avec ce système de génération était la nécessité
...
On a donc des enregistrements DNS, DHCP et netbox pour ces master, des VM qui
...
Cependant, le vrai problème est celui de la gestion de l'IaC.

Diabolocom utilise Ansible intensément pour toute la gestion de l'infra. Un
"problème" connu et inhérent au fonctionnement d'Ansible est la gestion de la
suppression.

Ansible est stateless, contrairement à terraform par exemple. Dans l'idée, on
défini une ressource dans un état défini, mais les ressources qui ne sont pas
défini par Ansible sont ignorées. C'est un avantage, mais aussi un inconvénient.
...
souhaitez avec Ansible ajouter le vôtre, vous pouvez utiliser le module `user`
ainsi :
...
Cette tâche Ansible va créer l'utilisateur zarak si il n'existe pas, ou ne rien
faire si il existe déjà.
...
Maintenant imaginons que cette étape de suppression explicite n'aie pas été
écrite lors d'un gros changement sur notre Ansible : l'ancienne version existera
...
ou non, peut importe) qui ne serait jamais désinstallé, etc.
{: .notice--warning }

### Solutions apportées par Vauban

L'idée de vauban est de repartir systématiquement de zéro (depuis une image officielle
raw de debian), et d'y appliquer les playbooks Ansible nécéssaire pour arriver
à l'image désirée. Il n'y a pas d'éléments stateful, de restes d'une désinstallation
partielle, ... Pas besoin également d'avoir une VM qui tourne pour ça. Grâce à
`chroot(1)` par exemple -- pour ne citer que lui -- nous pouvons créer des images
n'importe-où : en local sur nos machines, ou bien dans gitlab-CI.

D'autres éléments pertinents dans le cadre de la gestion des images sont également
proposés par Vauban, et seront présentés plus en détails par la suite.
...
Bien que non-nécessaire pour Vauban, c'est la méthode qui avait été retenue
par les équipes précédentes pour gérer les VM et serveurs. Vauban doit donc
pouvoir génerer des images qui fonctionnent en PXE.
...
jamais été testé, et de la configuration précise pour le boot PXE est apporté dans
le projet
{: .notice--info }

Sans rentrer dans tous les détails de PXE, l'idée est de pouvoir boot en réseau.
...
En PXE, vous ne chargez non pas sur un disque local, mais sur des ressources en
...
instructions spécifique à PXE (toujours via DHCP) concernant l'adresse d'un
serveur à contacter et d'un fichier à télécharger (en TFTP par exemple).

L'ordinateur s'occuper de télécharger ce fichier, notre bootloader réseau, qui
va ensuite s'occuper de télécharger notre kernel (vmlinuz) et un initramfs (voir
plus bas).
...
L'idée derrière est du coup la possibilité de ne pas avoir de disque dur dans la
machine par exemple, mais aussi de pouvoir centraliser en un point unique ce sur
quoi les machines vont boot, etc.

Le fait de ne pas avoir de disque dur pour backer notre OS signifie que l'OS sera
en RAM. Tous les changements qui ne seront pas sauvegardé sur un stockage externe
seront perdus.
{: .notice--info }

Le PXE est utilisé notamment à EPITA pour les workstations des étudiants. Le CRI
...
grâce à ça, pour éviter les "ça marche sur telle machine mais pas sur celle-là", ou
éviter la triche en examen.
{: .notice--success }
...
Le rootfs est notre OS à proprement parler, c'est ce qui sera `mount` sur `/`.
Si l'on peut avoir `/home` ou `/var` par exemple mounté sur d'autres disques,
`/` (donc votre rootfs) doit contenir le minimum d'un OS classique (Debian par
exemple), donc `/bin`, `/etc/`, etc ...
...
Lors du processus de boot, le kernel est chargée en mémoire, ainsi que l'initramfs
(d'où le RAM dans `initramfs`). Le kernel s'occupe de setup tout un tas de choses
qui ne seront pas détaillées ici, puis appelle le processus `init` de l'initramfs.

L'initramfs exécute le processus init, "fait des choses", et passe le flambeau
au rootfs une fois qu'il a fini. Une fois que le processus init du _rootfs_ a fini
de préparer les dernières choses et de lancer les services à lancer, on peut
considérer le processus de boot comme terminé.

Le processus init sur les distributions les plus répandues est le plus souvent systemd.
{: .notice--info }

Systemd au lancement s'occupe -- entre autres -- de lancer les services qui ont
été `enabled`, de lancer les unités `.mount`, certaines `.target`, etc ...
Il est possible d'obtenir quelques infos avec `sudo systemctl list-units --all`,
`sudo systemctl list-unit-files` et `sudo systemctl status`
{: .notice--info }

### L'initramfs

L'initramfs est une machinerie qui peut sembler au premier abord assez complexe
et floue. Son rôle est de _préparer tout ce qui est nécessaire pour donner la
main au rootfs_.
...
- Comment boot sur une partition/rootfs chiffrée ?
  ...
  Linux n'est pas capable de déchiffrer "tout seul" une partition chiffré, il
  semble donc difficile de trouver sur une partion chiffré comment boot dessus.
  L'initramfs intervient ici en étant lui non-chiffré (linux peut ainsi le lancer),
  et en ayant un init process capable de demander un mot de passe à l'utilisateur,
  et de l'utiliser pour déchiffrer le rootfs

- Comment boot sur un filesystem exotique ?

  Si pour une raison ou pour une autre on décide d'utiliser autre chose comme
  filesystem que ceux supportés par Linux (ext4, xfs, ...), on peut avoir besoin
...
- Comment boot sur un rootfs situé sur le réseau ?
  ...
  Par exemple, si par _le plus grand des hasards_, on cherche à booter sur un
  ...
  torrent, ...), puis savoir quoi faire avec le rootfs téléchargé avant de lui
  donner la main.
...
On également trouver la dénomination `initrd`. Bien qu'il existe quelques
...
- l'initrd est un block device en RAM, nécessite donc d'être `mount` et de
  posséder un filesystem (ext4 par exemple).
- l'initramfs est une archive, au format `cpio(1)`. Il suffit donc d'extraire
  ...
  tmpfs étant utilisé ici et étant un filesystem virtuel purement en RAM, dans le
  kernel
...
normal, non privilégié, et la création de char device avec `mknodat` n'est pas
...
$ file init
init: Neil Browns ash script, ASCII text exécutable
...
du quel on va installer nos services et créer nos composants. A diabolocom, nous
...
Pour commencer, il faut pouvoir "attaquer" notre image raw téléchargée. Cette
image est un block device contenant plusieurs partitions. Pour y accéder, il est
nécessaire de créer un loop device avec `losetup(8)`, puis de mounter la partition
...
donc nécessaire d'effectuer une sauvegarde du fichier "intact" au préalable.
...
nécessaire au boot (certes, le plus simple à obtenir), yay !

Créons donc notre initramfs à proprement parler, en utilisant `dracut`.

#### chroot_dracut

Pour utiliser dracut dans notre image, le plus simple reste encore de chroot
dedans, de l'installer, le configurer et de le lancer.

Utilisons `chroot(1)` pour installer les paquets Debian pour dracut.

Il faut faire attention aux noms de paquet qui change de la version 10 à 11.
...
Nous pouvons ensuite position le `dracut.conf` suivant dans notre FS :
...
être nécessaire des les y télécharger/copier depuis [le repo officiel de dracut](https://github.com/dracutdevs/dracut/tree/master/modules.d)
...
concernent pas, puisque notre setup n'utilisent ni NFS, ni BTRFS, ni LVM, ni
chiffrement, ...

On demande les drivers utiles pour le réseau (e1000e) et ceux qui nous serviront
pour notre rootfs (voir plus bas)

Pour finir, on demande à dracut de générer un initramfs qui sera générique, et
non pas dédiée à notre machine actuelle (comportement par défaut).
...
avec lequel on tourner n'est pas nécessairement celui contre lequel on cherche
...
chercher à génerer un initramfs pour le kernel 5.15.7.
{: .notice--warning }
...
Notre initramfs peut également être configuré plus en profondeur avec le fichier
`dracut.conf`, mais aussi via les modules. La force de dracut est sa modularité.
...
binaire `cryptsetup` -- nécessaire pour déchiffrer le disque -- est présent sur
...
utilisons depuis tout à l'heure est en réalité notre rootfs. Elle contient
en effet notre OS, et se mount sur `/`.

C'est donc de cette image qu'il faut partir. Cependant, l'image seule bien
évidemment sera un peu légère. Il faut y ajouter toute notre configuration, nos
paquets, etc.

A la manière des anciens `masters` qui étaient des VM, nous allons chercher à
générer un "master", c'est à dire un type de rootfs, par rôle. Notre ancien
master-postgresql deviendra donc une représentation virtuelle (par opposition à
...
En d'autre termes, un master avec vauban est un rootfs qui contient notre
configuration et nos programmes pour un usage précis (un rootfs pour postgresql),
pour un type de VM précis, mais c'est également la description de comment arriver
à ce rootfs, comment le génerer.

#### Approche hiérarchique

En réfléchissant un peu plus, on se rend compte que **toutes** nos VM possèdent une
base communes :
- des paquets de bases installés partout
...
"de base" comme base de toutes les autres images.

Puisque nous avons défini qu'un master était défini par un rootfs initial, il
nous suffit de considérer qu'au lieu de partir du rootfs fourni par debian
pour installer notre postgresql, on parte de notre image "de base" pour ensuite
ajouter postgresql.
...
dépendant lui-même du master `master-11-base`, etc.
...
master `master-11-base` et d'y appliquer le playbook ansible `playbook_docker.yml`.
...
nécessaire de reconstruire l'intrégralité de l'arbre occasionnellement ou dans
certains cas. Analoguement, une mise à jour dans le playbook_base.yml entraine
la nécessité de reconstruire `master-11-base`, mais également tous les fils qui
en dépendent, pour propager les mises à jour.
{: .notice--warning }

#### Et la configuration ?

S'il est bien beau de parler de master commun à un regroupement de machines,
il arrive forcément le moment où 2 VM, disons 2 servers postgresql, auront
besoin de se distinguer l'une de l'autre.

Bien qu'elle partageront 99% de leur rootfs en commun, auront le même OS et les
...
du rootfs en commun par autant de machines semblent nous indiquer que cette
solution est sub-optimale. Elle l'est d'autant plus lorsque l'on considère la
taille d'un rootfs : environ 800 MiB pour un rootfs avec postgresql pour quelques
...
Il suffirait "juste" de faire la différences entre le produit fini et le produit
...
configurations de toutes les machines tirée d'un master sur ledit master.<br/>
...
donc nécessairement un service au démarrage de la machine qui s'occupait de
faire un softlink du fichier de config `pg_hab.conf.<hostname>` à `pg_hba.conf`.<br/>
...
tirée du master, donc 50x plus de temps), toutes les machines avaient les
...
pensez à `overlayfs`, félicitations, c'est effectivement comme ça que vauban
gère ses rootfs et conffs !

Plus exactement, vauban utilise docker. Pourquoi utiliser docker ?

- Docker offre naturellement un système de build hiérarchique
...
- Docker nous permet de stocker et déplacer nos rootfs et étapes intermédiaires
  facilement grâce au registry
- Docker nous offre du tooling autours du chroot et de l'exécution chrooté
  et isolé
...
python et ansible, en positionnant la clé SSH de déploiement d'ansible (utilisée
pour clone le repo).

Après un `docker build`, nous disposons donc d'une image correspondant quasiment
...
appliquer pour Ansible se traduit par une liste de "stages", des playbooks
ansible, à appliquer successivement.
...
- `hostname` : Le hostname à définir dans notre `docker build`, pour simuler
   ...
   le bon groupe, les bonnes variables, etc)
...
La syntaxe est donc `[<branch>@]<playbook>`.

Le prefix commun quant à lui est la manière de dénommer chacune des étapes
de création de l'image finale. Chaque `stage` à appliquer créer en réalité une
...
Les images `master-11-docker/playbook_elastic.yml` et `master-11-elastic` sont
en réalité deux labels différents sur la même image docker. En effet,
`master-11-elastic` n'est rien d'autre de plus que l'image `master-11-docker`
...
L'image créé est bien celle expliquée plus haut : `$prefix_name/$playbook`.
...
positionne sur la branch qui a été demandée.
...
Cette manière d'appliquer Ansible présentent quelques désagréments. Voir la
section approprié de ce billet.
{: .notice--warning }
...
nous permet d'exporter sous le format `tar(1)` l'intégralité de notre rootfs.
...
format retenu est le `squashfs`, qui est parfaitement adapté à ce genre
...
Cette étape est en réalité plus rapide malgré le nombre de hosts que le rootfs,
...
cette expression correspond à celle qu'on utiliserait avec l'option `--limit`
d'`ansible-playbook`. La syntaxe est exactement la même -- assez logiquement
puisque la chaîne est envoyée à ansible-playbook pour qu'il l'interprete.

Ainsi on pourrait écrire `postgresql*,*-postgresql*,!postgresql9*` par exemple
pour matcher de manière plus sélective les hostnames qui sont plaisent.
...
Si les notions d'overlayfs, de layer et de diff ne sont pas claires, la
documentation de docker fourni des explications assez détaillées et compréhensibles.
{: .notice }
...
Si on regarde donc la dernière couche de notre image généré pour un host précis,
qui correspond ici à l'application d'un playbook pour configurer elasticsearch,
on peut voir que docker nous donne le dossier de la layer, et ce dernier ne
...
S'il n'y a pas grand chose à dire sur le fonctionnement en elle même de la
fonction, il y a cependant un mécanisme pour gérer le matching entre version du
...
Le dossier `linux` contient les kernels et initramfs, suffixé par leur version,
et les dossier des masters font un symlink sur la version qui les concernent.
...
Pour cela, toutes les commandes de `docker` sont en réalité wrappé dans une
fonction de `utils.sh`, qui s'occupe de :
- Se connecter automatiquement au registry avec les valeurs défini dans la
...
La question qui se posent désormais, c'est comment booter sur ces ressources,
...
La gestion de cette configuration est laissée à la discretion du lecteur.
{: .notice }

Bon la configuration semble un peu velue lors de la première approche, certes.

L'idée de base est d'indiqué via les directives `linux` et `initrd` où trouver
...
l'infra, la variable `net_default_mac` doit en revanche être changée d'une
machine à une autre, assez logiquement.
...
trop contraignantes, il va falloir ruser.
...
Si ça semble familier, c'est normal, je décris une fois de plus overlayfs.
...
à boot, grâce à overlayfs. Il suffit de regarder le contenu de l'`upperdir`
{: .notice--info }
...
Ces paramètres ne sont pas propres à un master donnée comme peuvent l'être les
...
Une variable défini dans `.secrets.env` ne peut pas être overwrite avec une variable
d'environnement.
{: .notice--warning }
...
reproductibles et hiérarchique, un des inconvénient majeur est l'application
...
Par ailleurs, systemd n'est pas l'init process du container (ne peut pas l'être).
...
bootées en PXE, il est possible d'avoir du stockage persistant en mountant
...
Cette opération ne peut être faite directement depuis vauban.
